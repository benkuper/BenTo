  ...
  if (list == NULL || *list == '\0') {
    ...
    if (eq_val != NULL) {
      ...
      if (eq_val->ptr != NULL) {
        ...
        val->len = (int) ((eq_val->ptr - val->ptr) - 1);
      }
    }
  }
...
static void spawn_stdio_thread(sock_t sock, HANDLE hPipe,
                               void *(*func)(void *)) {
  struct threadparam *tp = (threadparam *)malloc(sizeof(*tp));
  if (tp != NULL) {
    tp->s = sock;
    tp->hPipe = hPipe;
...
static void open_cgi_endpoint(struct connection *conn, const char *prog) {
  ...
  if ((p = strrchr(dir, '/')) != NULL) {
    *p++ = '\0';
    ...
    static_cast<void>(dir[0] = '.'), dir[1] = '\0';
    p = (char *) prog;
  }
...
static void read_from_cgi(struct connection *conn) {
  ...
  int len, s_len = sizeof(cgi_status) - 1,
      n = (int) recv(conn->endpoint.cgi_sock, buf, sizeof(buf), 0);
...
static int convert_uri_to_file_name(struct connection *conn, char *buf,
  ...
  for (p = buf + strlen(root) + 2; *p != '\0'; p++) {
    if (*p == '/') {
      *p = '\0';
      if (match_prefix(cgi_pat, (int) strlen(cgi_pat), buf) > 0 && !stat(buf, st)) {
      DBG(("!!!! [%s]", buf));
        *p = '/';
...
static void send_websocket_handshake(struct mg_connection *conn,
  ...
  mg_write(conn, buf, (int) strlen(buf));
}
...
int mg_websocket_write(struct mg_connection* conn, int opcode,
    ...
    if (copy_len > 0) {
      retval = mg_write(conn, copy, (int) copy_len);
    }
    free(copy);
...
static void send_directory_listing(struct connection *conn, const char *dir) {
  ...
  spool(&conn->remote_iobuf, buf, (int) strlen(buf));
...
int parse_header(const char *str, int str_len, const char *var_name, char *buf,
                 size_t buf_size) {
  int ch = ' ', len = 0, n = (int) strlen(var_name);
...
int mg_parse_header(const char *s, const char *var_name, char *buf,
                    size_t buf_size) {
  return parse_header(s, s == NULL ? 0 : (int) strlen(s), var_name, buf, buf_size);
}
...
static void open_local_endpoint(struct connection *conn) {
  ...
  if (!strcmp(conn->mg_conn.request_method, "OPTIONS")) {
    ...
    send_http_error(conn, 501, NULL);
#endif
  } else if (match_prefix(cgi_pat, (int) strlen(cgi_pat), path) > 0) {
#if !defined(NO_CGI)
...
static void read_from_socket(struct connection *conn) {
  ...
  if (conn->endpoint_type == EP_CLIENT) {
    ...
    if (conn->ssl != NULL) {
      ...
      n = (int) recv(conn->client_sock, buf, sizeof(buf), 0);
    }
...
static void execute_iteration(struct mg_server *server) {
  ...
  recv(server->ctl[1], (char *) msg, sizeof(msg), 0);
...
void mg_iterate_over_connections(struct mg_server *server, mg_handler_t handler,
  ...
  msg[0].f = handler;
  msg[1].p = param;
  send(server->ctl[0], (const char *) msg, sizeof(msg), 0);
}

...
void NodeConnectionManager::addItemsInternal(Array<NodeConnection*> items, var)
{
	for (auto& nc : items) nc->addConnectionListener(this);
}
...
void NodeConnectionManager::removeItemsInternal(Array<NodeConnection*> items)
{
	for (auto& nc : items) nc->removeConnectionListener(this);

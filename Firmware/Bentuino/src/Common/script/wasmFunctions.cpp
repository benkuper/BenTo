...
m3ApiRawFunction(m3_clearLeds)
{
    ...
    //RootComponent::instance->strip.clearColors();
    m3ApiSuccess();
}
...
m3ApiRawFunction(m3_getOrientation)
{
    ...
    // float v = oi < 3 ? RootComponent::instance->imu->orientation[oi] : -1;
...
{
    m3ApiReturnType(float);
    m3ApiReturn(RootComponent::instance->imu->orientation[0]);
}

m3ApiRawFunction(m3_getPitch)
{
    m3ApiReturnType(float);
    m3ApiReturn(RootComponent::instance->imu->orientation[1]);
}
m3ApiRawFunction(m3_getRoll)
{
    m3ApiReturnType(float);
    m3ApiReturn(RootComponent::instance->imu->orientation[2]);
}
...
{
    m3ApiReturnType(uint32_t);
    m3ApiReturn((uint32_t)RootComponent::instance->imu->throwState);
}
...
{
    m3ApiReturnType(float);
    m3ApiReturn(RootComponent::instance->imu->projectedAngle);
}
...
m3ApiRawFunction(m3_setProjectedAngleOffset)
{
    ...
    RootComponent::instance->imu->setProjectAngleOffset(yaw, angle);
    m3ApiSuccess();
}
...
m3ApiRawFunction(m3_setIMUEnabled)
{
    ...
    RootComponent::instance->imu->enabled->set((bool)en);
    m3ApiSuccess();
}

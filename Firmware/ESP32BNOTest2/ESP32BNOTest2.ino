...
void setup()
{
  ...
  Wire.begin(2,4); // (SDA, SCL) (21, 22) are default on ESP32, 400 kHz I2C bus speed
  //Wire.setClock(1000);
  delay(1000);
  Serial.begin(115200);
...
//  MahonyQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, mx, my, mz);
    ...
    delt_t = millis() - count;
    if (delt_t > 20) { // update LCD once per half-second independent of read rate
...
void magCalBNO055(float * dest1) 
{
  ...
  int mag_max[3] = {0x8000, 0x8000, 0x8000}, mag_min[3] = {0x7FFF, 0x7FFF, 0x7FFF};
...
// scan for i2c devices
  ...
  for(address = 1; address < 127; address++ ) 
  {
    ...
    //Wire.write(0x00); // Really needs this for reliability <<<---------------
